//
//  IslandNotchCustomizationStore.swift
//  ClaudeIsland
//
//  Central ObservableObject that holds the user's IslandNotchCustomization
...
@MainActor
final class IslandNotchCustomizationStore: ObservableObject {
    static let shared = IslandNotchCustomizationStore()
    ...
    @Published private(set) var customization: IslandNotchCustomization
    ...
    private var editDraftOrigin: IslandNotchCustomization?
    ...
    func update(_ mutation: (inout IslandNotchCustomization) -> Void) {
    ...
    private static func loadFromDefaults() -> IslandNotchCustomization? {
        guard let data = UserDefaults.standard.data(forKey: defaultsKey) else {
            return nil
        }
        return try? JSONDecoder().decode(IslandNotchCustomization.self, from: data)
    }

    // MARK: - Legacy migration

    /// Pull legacy @AppStorage values into a new IslandNotchCustomization.
    /// Does NOT mutate UserDefaults — deletion is a separate step that
    /// only runs after the v1 key is successfully written.
    private static func readLegacyOrDefault() -> IslandNotchCustomization {
        var c = IslandNotchCustomization.default
