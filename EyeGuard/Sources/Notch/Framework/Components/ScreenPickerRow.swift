...
struct IslandScreenPickerRow: View {
    @ObservedObject var screenSelector: IslandScreenSelector
        ...
        VStack(spacing: 0) {
            ...
            if isExpanded {
                VStack(spacing: 2) {
                    // Automatic option
                    ScreenOptionRow(
                        label: IslandL10n.automatic,
                        sublabel: IslandL10n.builtInOrMain,
    ...
    private func screenSublabel(for screen: NSScreen) -> String? {
        ...
        if screen.isBuiltinDisplay {
            parts.append(IslandL10n.builtIn)
        }
        if screen == NSScreen.main {
            parts.append(IslandL10n.main_)
        }
