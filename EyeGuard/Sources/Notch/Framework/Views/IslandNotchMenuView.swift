...
// MARK: - IslandNotchMenuView

struct IslandNotchMenuView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 4) {
                // Eye Guard panel shortcut (when eye guard mode is active)
                if AppDelegate.shared?.modeManager.isEyeGuardEnabled == true {
                    EyeGuardMenuRow(viewModel: viewModel)
                }
...
struct AccessibilityRow: View {
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            if isEnabled {
                Circle()
                    .fill(IslandTerminalColors.green)
                    .frame(width: 6, height: 6)

                Text(IslandL10n.enabled)
                    .font(.system(size: 11))
                    .foregroundColor(.white.opacity(0.4))
            } else {
                Button(action: openAccessibilitySettings) {
                    Text(IslandL10n.enable)
    ...
    var body: some View {
        Button(action: action) {
            HStack(spacing: 10) {
                ...
                Circle()
                    .fill(isOn ? IslandTerminalColors.green : Color.white.opacity(0.3))
                    .frame(width: 6, height: 6)

                Text(isOn ? IslandL10n.on : IslandL10n.off)
                    .font(.system(size: 11))
                    .foregroundColor(.white.opacity(0.4))
            }
            .padding(.horizontal, 12)
...
struct LanguageRow: View {
    ...
    @State private var isHovered = false
    @State private var current = IslandL10n.appLanguage
        ...
        VStack(spacing: 0) {
            Button {
                ...
                HStack(spacing: 10) {
                    ...
                    Text(IslandL10n.language)
                        .font(.system(size: 13, weight: .medium))
                        .foregroundColor(textColor)

                    Spacer()

                    Text(IslandL10n.currentLanguageLabel)
                        .font(.system(size: 11))
                        .foregroundColor(.white.opacity(0.4))
            ...
            if isExpanded {
                VStack(spacing: 0) {
                    ForEach(options, id: \.id) { option in
                        Button {
                            IslandL10n.appLanguage = option.id
                            current = option.id
                        } label: {
                            HStack {
                                Text(option.label)
...
struct ThresholdPickerRow: View {
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            Text(IslandL10n.alertThreshold)
                .font(.system(size: 13, weight: .medium))
                .foregroundColor(textColor)
...
// MARK: - Mode Switch Row

struct ModeSwitchRow: View {
    @State private var isHovered = false

    private var modeManager: ModeManager? {
        AppDelegate.shared?.modeManager
    }

    private var currentMode: AppMode {
        modeManager?.currentMode ?? .dual
    }
...
// MARK: - Eye Guard Menu Row

struct EyeGuardMenuRow: View {
    @ObservedObject var viewModel: IslandNotchViewModel
    @State private var isHovered = false

    private var module: EyeGuardModule? {
        AppDelegate.shared?.eyeGuardModule
    }

    var body: some View {
        Button {
            if AppDelegate.shared?.modeManager.currentMode == .dual {
                viewModel.contentType = .dualMode
            } else {
                viewModel.contentType = .eyeGuard
            }
        } label: {
            HStack(spacing: 10) {
                ...
                if let mod = module {
                    Circle()
                        .fill(mod.isActive ? IslandTerminalColors.green : .white.opacity(0.3))
                        .frame(width: 6, height: 6)

                    Text("\(mod.healthScore)")
                        .font(.system(size: 11))
                        .foregroundColor(.white.opacity(0.4))
                }
            }
