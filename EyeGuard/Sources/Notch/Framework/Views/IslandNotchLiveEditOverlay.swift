//
//  IslandNotchLiveEditOverlay.swift
//  ClaudeIsland
//
//  SwiftUI content for the IslandNotchLiveEditPanel. Lays out floating
...
//  - The visible notch is rendered by IslandNotchView at screen-mid plus
...
struct IslandNotchLiveEditOverlay: View {
    let screenID: String
    ...
    /// `IslandNotchHardwareDetector.hasHardwareNotch(...)` return false
    ...
    private var visibleNotchHeight: CGFloat {
        ...
        return IslandNotchHardwareDetector.clampedHeight(geo.notchHeight)
    }
    ...
    private var baseNotchWidth: CGFloat {
        let hw = IslandNotchHardwareDetector.hardwareNotchWidth(
            on: screenProvider(),
    ...
    private var hasHardwareNotch: Bool {
        IslandNotchHardwareDetector.hasHardwareNotch(
            on: screenProvider(),
            mode: store.customization.hardwareNotchMode
        )
    }
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            // Mirror the same clamp IslandNotchView applies for `.offset(x:)`.
            ...
            ZStack(alignment: .topLeading) {
                ...
                if presetMarkerVisible && hasHardwareNotch {
                    ...
                    let markerWidth = IslandNotchHardwareDetector.hardwareNotchWidth(
                        on: screenProvider(),
                        mode: store.customization.hardwareNotchMode
                    )
                    Rectangle()
                ...
                VStack(spacing: 10) {
                    HStack(spacing: 10) {
                        actionButton(
                            title: IslandL10n.notchEditNotchPreset,
                            icon: "scope",
                            enabled: hasHardwareNotch,
                            tooltip: hasHardwareNotch ? nil : IslandL10n.notchEditPresetDisabledTooltip
                        ) {
                            applyNotchPreset()
                        }
                        ...
                        actionButton(
                            title: IslandL10n.notchEditDragMode,
                            icon: "hand.draw",
                            enabled: true,
                        ...
                        actionButton(
                            title: IslandL10n.notchEditReset,
                        ...
                        Button {
                            ...
                            HStack(spacing: 5) {
                                Image(systemName: "xmark")
                                    .font(.system(size: 11, weight: .heavy))
                                Text(IslandL10n.notchEditCancel)
                                    .font(.system(size: 12, weight: .bold))
                            }
                            .foregroundColor(.black)
                            .padding(.horizontal, 22)
    ...
    private func applyHeightStep(direction: Int) {
        ...
        store.updateGeometry(for: screenID) { geo in
            geo.notchHeight = IslandNotchHardwareDetector.clampedHeight(
    ...
    private func applyArrowStep(direction: Int) {
        ...
        store.updateGeometry(for: screenID) { geo in
            geo.maxWidth = max(
                IslandNotchHardwareDetector.minIdleWidth,
    ...
    private func applyNotchPreset() {
        let width = IslandNotchHardwareDetector.hardwareNotchWidth(
            on: screenProvider(),
            mode: store.customization.hardwareNotchMode
        )
