//
//  IslandNotchCustomizationSettingsView.swift
//  ClaudeIsland
//
...
struct IslandNotchCustomizationSettingsView: View {
    @ObservedObject private var store: IslandNotchCustomizationStore = .shared
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 8) {
                visibilityToggle(
                    icon: "sparkles",
                    label: IslandL10n.notchShowBuddy,
                    isOn: store.customization.showBuddy
                ) {
                    store.update { $0.showBuddy.toggle() }
                }
                .accessibilityLabel(IslandL10n.notchShowBuddy)

                visibilityToggle(
                    icon: "chart.bar.fill",
                    label: IslandL10n.notchShowUsageBar,
                    isOn: store.customization.showUsageBar
                ) {
                    store.update { $0.showUsageBar.toggle() }
                }
                .accessibilityLabel(IslandL10n.notchShowUsageBar)
            }
    ...
    private var themeRow: some View {
        controlRow(icon: "paintpalette", label: IslandL10n.notchTheme) {
            Menu {
                ForEach(NotchThemeID.allCases) { id in
                    Button {
                        ...
                        Label {
                            Text(IslandL10n.notchThemeName(id))
                        } icon: {
                            Circle().fill(NotchPalette.for(id).bg)
                        }
                        .accessibilityLabel("\(IslandL10n.notchThemeName(id)) theme")
                    }
                }
            } label: {
                HStack(spacing: 6) {
                    ...
                    Text(IslandL10n.notchThemeName(store.customization.theme))
            ...
            .fixedSize()
            .accessibilityLabel(IslandL10n.notchTheme)
        }
    }
    ...
    private var fontSizeRow: some View {
        controlRow(icon: "textformat.size", label: IslandL10n.notchFontSize) {
            HStack(spacing: 0) {
                fontSizeSegment(.small,   shortLabel: IslandL10n.notchFontSmall,   accessibilityLabel: IslandL10n.notchFontSmallFull)
                fontSizeSegment(.default, shortLabel: IslandL10n.notchFontDefault, accessibilityLabel: IslandL10n.notchFontDefaultFull)
                fontSizeSegment(.large,   shortLabel: IslandL10n.notchFontLarge,   accessibilityLabel: IslandL10n.notchFontLargeFull)
                fontSizeSegment(.xLarge,  shortLabel: IslandL10n.notchFontXLarge,  accessibilityLabel: IslandL10n.notchFontXLargeFull)
            }
            .padding(2)
            .background(
    ...
    private var hoverSpeedRow: some View {
        controlRow(icon: "cursorarrow.motionlines", label: IslandL10n.notchHoverSpeed) {
            HStack(spacing: 0) {
                hoverSpeedSegment(.instant, shortLabel: IslandL10n.notchHoverInstant)
                hoverSpeedSegment(.normal,  shortLabel: IslandL10n.notchHoverNormal)
                hoverSpeedSegment(.slow,    shortLabel: IslandL10n.notchHoverSlow)
            }
            .padding(2)
            .background(
                RoundedRectangle(cornerRadius: 6).fill(Color.white.opacity(0.06))
    ...
    private var hardwareModeRow: some View {
        controlRow(icon: "laptopcomputer", label: IslandL10n.notchHardwareMode) {
            Menu {
                Button(IslandL10n.notchHardwareAuto) {
                    store.update { $0.hardwareNotchMode = .auto }
                }
                Button(IslandL10n.notchHardwareForceVirtual) {
                ...
                HStack(spacing: 6) {
                    Text(
                        store.customization.hardwareNotchMode == .auto
                            ? IslandL10n.notchHardwareAuto
                            : IslandL10n.notchHardwareForceVirtual
                    )
                    .font(.system(size: 12, weight: .medium))
                    .foregroundColor(.white.opacity(0.95))
                    Image(systemName: "chevron.up.chevron.down")
            ...
            .menuIndicator(.hidden)
            .fixedSize()
            .accessibilityLabel(IslandL10n.notchHardwareMode)
        }
    }
    ...
    private var customizeButton: some View {
        Button {
            ...
            HStack(spacing: 8) {
                ...
                Text(IslandL10n.notchCustomizeButton)
                    .font(.system(size: 12, weight: .semibold))
                Spacer(minLength: 0)
        ...
        .buttonStyle(.plain)
        .accessibilityLabel(IslandL10n.notchCustomizeButton)
