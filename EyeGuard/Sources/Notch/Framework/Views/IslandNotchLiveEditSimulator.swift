//
//  IslandNotchLiveEditSimulator.swift
//  ClaudeIsland
//
...
enum IslandNotchLiveEditSimulator {
...
struct NotchLiveEditSimulatorView: View {
    ...
    var body: some View {
        TimelineView(.periodic(from: .now, by: 2)) { context in
            let text = IslandNotchLiveEditSimulator.fixtures[index]
            Text(text.isEmpty ? " " : text)
                ...
                .onChange(of: context.date) { _, newDate in
                    ...
                    index = (index + 1) % IslandNotchLiveEditSimulator.fixtures.count
                }
