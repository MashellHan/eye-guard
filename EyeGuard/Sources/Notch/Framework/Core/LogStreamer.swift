...
@MainActor
final class IslandLogStreamer: ObservableObject {
    static let shared = IslandLogStreamer()
    ...
    private func installWatcher() {
        ...
        guard fd >= 0 else {
            IslandDebugLogger.log("IslandLogStreamer", "open() failed for \(logPath) errno=\(errno)")
            return
        }
