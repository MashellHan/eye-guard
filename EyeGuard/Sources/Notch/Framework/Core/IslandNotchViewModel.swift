...
enum IslandNotchContentType: Equatable {
    ...
    case eyeGuard        // Eye Guard panel
    case dualMode        // Dual mode split panel
    ...
    var id: String {
        ...
        case .eyeGuard: return "eyeGuard"
        case .dualMode: return "dualMode"
        }
    }
}
...
class IslandNotchViewModel: ObservableObject {
    ...
    private let soundSelector = IslandSoundSelector.shared

    // MARK: - Geometry

    let geometry: IslandNotchGeometry
    let spacing: CGFloat = 12
    let hasPhysicalNotch: Bool
    let screenID: String

    /// Current expansion width from IslandNotchView (synced for hit testing)
    ...
    private let events = IslandEventMonitors.shared
    ...
    init(deviceNotchRect: CGRect, screenRect: CGRect, windowHeight: CGFloat, hasPhysicalNotch: Bool, screenID: String) {
        ...
        self.geometry = IslandNotchGeometry(
    ...
    /// clamp IslandNotchView applies for `.offset(x:)` rendering.
    private var currentHorizontalOffset: CGFloat {
        let geo = IslandNotchCustomizationStore.shared.customization.geometry(for: screenID)
        let runtime: CGFloat = status == .opened ? openedSize.width : (geometry.deviceNotchRect.width + currentExpansionWidth)
        return IslandNotchHardwareDetector.clampedHorizontalOffset(
    ...
    private func handleMouseMove(_ location: CGPoint) {
        ...
        if IslandNotchCustomizationStore.shared.isEditing {
            isHovering = false
            hoverTimer?.cancel()
            hoverTimer = nil
            return
        ...
        if isHovering && (status == .closed || status == .popping) {
            let delay = IslandNotchCustomizationStore.shared.customization.hoverSpeed.delay
            if delay <= 0 {
