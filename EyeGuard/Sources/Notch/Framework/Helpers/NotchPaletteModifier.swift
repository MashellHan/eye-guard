//
//  IslandNotchPaletteModifier.swift
//  ClaudeIsland
//
...
//  uses an imperatively-created `NSHostingView<IslandNotchView>` with a
//  strictly-typed generic, adding `.environmentObject(_:)` would
//  change the generic type of the root view and break the
//  `PassThroughHostingView<IslandNotchView>` declaration. As a pragmatic
//  alternative we observe `IslandNotchCustomizationStore.shared`
...
struct IslandNotchPaletteModifier: ViewModifier {
...
extension View {
    ...
    /// IslandNotchView root. Animates across 0.3s on theme change.
    func notchPalette() -> some View { modifier(IslandNotchPaletteModifier()) }
